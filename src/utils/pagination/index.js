...
const pagination = ({ total = 1, activePage = 1 } = {}) => {
    if(!isNumber(total)){
        throw new TypeError('total deve ser um numero')
    }
    if(!isNumber(activePage)){
        throw new TypeError('activePage deve ser um numero')
    }
    // if(activePage > total){
    //     throw new TypeError('activePage não pode ser maior que o total de páginas')
    // }
    if (total <= 5) {
